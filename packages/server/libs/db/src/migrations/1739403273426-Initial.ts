...
export class Initial1739403273426 implements MigrationInterface {
  name = "Initial1739403273426";

  public async up(queryRunner: QueryRunner,): Promise<void> {
    await queryRunner.query("CREATE TABLE \"token\" (\"createdAt\" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), \"updatedAt\" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), \"id\" SERIAL NOT NULL, \"address\" character varying(42) NOT NULL, \"chainId\" integer NOT NULL, \"decimals\" integer NOT NULL, \"symbol\" character varying NOT NULL, \"name\" character varying NOT NULL, \"usdPrice\" double precision NOT NULL, \"marketCap\" double precision NOT NULL, \"iconUrl\" character varying, CONSTRAINT \"UQ_294a189de3c92b7ec61d926343f\" UNIQUE (\"address\", \"chainId\"), CONSTRAINT \"PK_82fae97f905930df5d62a702fc9\" PRIMARY KEY (\"id\"))",);
    ...
    await queryRunner.query("CREATE TABLE \"sync_metadata\" (\"key\" character varying(32) NOT NULL, \"updatedAt\" TIMESTAMP WITH TIME ZONE NOT NULL, CONSTRAINT \"PK_b15549beeeaebd13e989c638ff6\" PRIMARY KEY (\"key\"))",);
    await queryRunner.query("ALTER TABLE \"supported_token\" ADD CONSTRAINT \"FK_060e1d69c71900a6c55ac3c5dbf\" FOREIGN KEY (\"providerKey\") REFERENCES \"provider\"(\"key\") ON DELETE CASCADE ON UPDATE NO ACTION",);
    await queryRunner.query("ALTER TABLE \"supported_token\" ADD CONSTRAINT \"FK_13878666830d772c121bf29d556\" FOREIGN KEY (\"tokenId\") REFERENCES \"token\"(\"id\") ON DELETE CASCADE ON UPDATE NO ACTION",);
    await queryRunner.query("ALTER TABLE \"payment_option\" ADD CONSTRAINT \"FK_708a9468eaf199a487d3912f7de\" FOREIGN KEY (\"providerKey\") REFERENCES \"provider\"(\"key\") ON DELETE CASCADE ON UPDATE NO ACTION",);
  }
