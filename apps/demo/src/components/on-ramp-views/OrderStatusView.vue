<template>
  <div class="flex flex-col gap-2 h-full">
    <PanelHeader title="Status" back="buy">
      <button @click="stopRoute()" type="button" class="cursor-pointer shrink rounded-full px-3 p-2">
        <Icon icon="fluent:record-stop-32-filled" class="w-6 h-6 text-red-600/50 hover:text-red-600" />
      </button>
    </PanelHeader>
    <div class="relative h-full overflow-y-auto">
      <div class="flex flex-col items-center pt-8">
        <VueSpinnerGears v-if="orderStatus === 'IN_PROGRESS' || orderStatus === 'PENDING'" size="36" color="#2b7fff" />
        <div v-if="orderStatus === 'DONE'" ><!--   -->
          <Icon v-if="!error && isReady" icon="fluent:checkmark-circle-32-regular" class="w-10 h-10 text-green-700" />
          <Icon v-if="error && isReady" icon="fluent:error-circle-24-regular" class="w-10 h-10 text-orange-500"/>
        </div>
        <div v-if="orderStatus === 'STOPPED'"><!--   -->
          <Icon icon="fluent:error-circle-24-regular" class="w-10 h-10 text-orange-500"/>
        </div>
      </div>
...
const {
  order, error, isReady, inProgress, orderStatus,
