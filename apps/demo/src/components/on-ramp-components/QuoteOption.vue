<template>
  <div class="border border-gray-200 rounded-2xl p-4 shadow hover:shadow-md hover:border-gray-300 cursor-pointer" @click="executeQuote">
    <div class="flex gap-2 items-center relative">
      ...
      <div class="flex flex-col gap-0.5">
        ...
        <span class="text-sm text-gray-600">
          {{ paymentMethod }} via <span class="font-semibold">{{ quote.provider.name }}</span>
        </span>
      </div>
...
const receiveAmount = computed(() => {
  const amount = formatUnits(parseUnits(props.quote.receive.amountUnits, 0), props.quote.receive.token.decimals);
