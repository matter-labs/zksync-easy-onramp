<template>
  <div
    ...
    class="cursor-pointer border border-gray-400 rounded-2xl flex gap-2 items-center p-2 hover:bg-gray-100 hover:border-gray-500"
  >
    <div>
      <TokenIcon
        :token="route.receive.token"
        class="w-10 h-10"
      />
    </div>
...
const lastMessage = computed(() => {
  ...
  return lastStep.execution?.process[lastStep.execution.process.length - 1]
    .message;
});
...
const payAmount = computed(() => {
  if (props.route) {
    return formatFiat(
      props.route.pay.fiatAmount ?? 0,
      props.route.pay.currency,
    );
  }
  return null;
});
...
const receiveAmount = computed(() => {
  if (props.route) {
    return formatToken(
      props.route.receive.amountUnits ?? 0,
      props.route.receive.token.decimals,
    );
  }
  return null;
});
